---
title: "Week 3 Analysis: “All circles are equal, but...” (Part 1)"
...
Today’s post really got away from me, so I’m splitting it into two parts. The data I recorded and used is [here](link to data page), and my .Rmd file is [here](link to github), for those of you who are interested.
...
Remember, **teams that are above the line are doing better than expected**, and teams below the line, worse. Endemic’s performance really stands out to me, as well as SSG’s. Excelerate has improved, and Ghost has been very consistent. Apologies to Simplicity and Denial, who are right on top of each other. You can see this broken down by kills and placement [here](link to breakdown page)
...
```{r circle_importance_viz, echo = FALSE, results = "hide", fig.height = 12, fig.width = 7.5}
...
ridgeline_plot <- ggplot(ridgeline_data, 
     ...
     geom_density_ridges_gradient(scale = 1.45,
...
A ridgeline plot shows the density plot (basically a smoothed histogram) of the distribution of a variable, and compares different scenarios. Here, I plotted the distribution of points in all matches, separated out by luck with each circle, so **we can see the difference in results based on being favored or not favored by each circle**. The vertical **white lines are the average points per match** that teams ended up with if they were in, or not in, that particular circle. This type of plot might look unfamiliar, but it’s essentially a set of histograms, or sideways boxplots (or bee plots, or violin plots) stacked on top of each other.
...
Stay tuned for Part 2 next week, where I will try to answer the question: **how many additional points do you get for being in any given circle?**

&#x200B;

**tl;dr Circle 4 appears to be the most influential, late game circles do have more of an impact, and Endemic and Rumblers are doing particularly well.**